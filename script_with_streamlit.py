...
#################################
    ...
    st.write("Firstly I did a numerical encoding for  each variable, because in the original dataset features are indicated with categorical values, which is not useful to continue with analysis. ")
    
    with st.expander("Cleaned dataset features are as follows:"):
        cleaned_dataset = [
            "EthnicGroup and ParentMaritalStatus: I used a different approach, which is One-hot encoding: Each ethnic group has a different binary variable.",
            "Gender: Binary encoding: 1 for female, 0 for male.",
            "ParentEduc: Numerical encoding from 0 to 3 for different education levels (0 is lower and 3 is higher)",
            "LunchType: Binary encoding: 0 for standard, 1 for free/reduced.",
        ...
        for cleaned_dataset in cleaned_dataset:
            st.write(cleaned_dataset)
    st.write("Then I impute missing values with Random Forest. Because of its efficiency and adaptability.")
...
#################################
    ...
    st.write("### Plot exam scores. Used two ways of visualization.")
...
#################################
    ...
    st.write('''Correlation comment: the most influencing features for scores are: gender,
            parent education, lunchtype, test preparation, sport practicing, weekly study hours, ethnic group.
    ...
    st.write("Here a detailed plot of some of the most influencing features.")
    
    # Plot correlation between Exam Scores and Gender 
    st.write('### Correlation between Exam Scores and Gender')
    ...
    # Plot correlation between Exam Scores and Practicing Sport
    st.write('### Correlation between Exam Scores and Practicing Sport')
    ...
    # Correlation between Exam Scores and Ethnic Group
    st.write('### Correlation between Exam Scores and Ethnic Group')
...
#################################
    ...
    # Second and Third Trial: PolyRegressor of degree=2 and degree=3 on Math Score
    # Create polynomial features, then reate and fit the polynomial regression model
